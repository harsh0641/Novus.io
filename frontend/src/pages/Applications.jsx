...
// Uses Groq Llama 3.1 API to generate context-aware requirement explanations per job
...
function JobModal({ job, onClose, onApply, isApplied, isSaving, dark, userId }) {
  ...
  // ADDED FUNCTIONALITY: Handle Groq AI fetching on hover
  const handleHoverSummary = async () => {
    setIsHoveringSum(true);
    ...
    >
      <div style={{ background: C.surface, borderRadius: 24, width: '100%', maxWidth: 680, maxHeight: '90vh', overflowY: 'auto', border: `1px solid ${C.border}`, boxShadow: dark ? '0 40px 100px rgba(0,0,0,0.6)' : '0 40px 100px rgba(0,0,0,0.12)', animation: 'modalIn 0.35s cubic-bezier(0.16, 1, 0.3, 1)' }}>
        ...
        <div style={{ padding: '20px 24px 28px' }}>
          ...
          {/* AI Summary — cleaned & ADDED dynamic hover interaction */}
          {summary && (
            <div 
              onMouseEnter={handleHoverSummary}
              onMouseLeave={() => setIsHoveringSum(false)}
              style={{ marginBottom: 22, padding: '18px', background: isHoveringSum ? C.surfaceL : C.surfaceLow, borderRadius: 16, border: `1px solid ${isHoveringSum ? '#3b82f6' : C.border}`, transition: 'all 0.3s ease', cursor: 'crosshair' }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <div style={{ fontSize: '0.58rem', fontWeight: 700, color: isHoveringSum ? '#3b82f6' : C.textL, letterSpacing: '0.12em', textTransform: 'uppercase', transition: 'color 0.3s' }}>
                  {isHoveringSum ? 'Live AI Analysis' : 'Intelligence Summary'}
                </div>
                {loadingSum && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.65rem', color: '#3b82f6', fontWeight: 600 }}>
                    <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#3b82f6', animation: 'reqPulse 1s infinite' }}/>
                    Fetching Deep Insights...
                  </div>
                )}
              </div>
              <p style={{ fontSize: '0.875rem', color: C.text, lineHeight: 1.75, margin: 0, fontWeight: 300, transition: 'opacity 0.3s', opacity: loadingSum ? 0.6 : 1 }}>
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: C.bg, transition: 'background 0.4s cubic-bezier(0.16,1,0.3,1)', fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif" }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, position: 'relative', zIndex: 10, marginLeft: 'var(--sidebar-width, 280px)', transition: 'margin-left 0.5s cubic-bezier(0.16, 1, 0.3, 1)' }} className="main-content">
        ...
        {/* ── TOP BAR (Added class app-header) ────────────────────────────── */}
        <header className="app-header" style={{ position: 'sticky', top: 0, zIndex: 40, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 24px', background: dark ? 'rgba(3,3,3,0.75)' : 'rgba(244,244,245,0.85)', backdropFilter: 'blur(24px) saturate(180%)', borderBottom: `1px solid ${C.border}`, WebkitBackdropFilter: 'blur(24px) saturate(180%)', gap: 16 }}>
          ...
          <div className="header-left" style={{ flex: 1, maxWidth: 480 }}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        /* ── Mobile Responsive (Appended matching behavior without removing rules) ── */
        @media (max-width: 768px) {
          .main-content {
            margin-left: 0 !important;
          }
          .mobile-only {
            display: none !important; /* Managed by global Sidebar component now to match Dashboard */
          }
          .header-user-info {
          ...
          /* Match Dashboard header completely */
          .app-header {
