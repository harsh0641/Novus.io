...
function getEngNum(job) { const s = String(job.applicants || job.ai_engagement || ''); const m = s.match(/\d+/); return m ? parseInt(m[0]) : 0; }
...
// Uses Groq via your backend /api/jobs/suggest-req endpoint (or inline Groq call)
...
async function fetchReqExplanation(req, jobTitle, company, jobDescription) {
  ...
  try {
    // Call Groq directly from the frontend using your backend proxy pattern
    // Since you don't have an Anthropic key, we call Groq via a simple fetch
    const GROQ_KEY = process.env.REACT_APP_GROQ_API_KEY || '';

    if (!GROQ_KEY) {
      // Fallback: generate a meaningful explanation based on the req text
      const fallback = `This skill is essential for ${jobTitle} at ${company} to ${req.toLowerCase().includes('communication') ? 'collaborate effectively with cross-functional teams' : req.toLowerCase().includes('python') || req.toLowerCase().includes('java') || req.toLowerCase().includes('react') ? 'build and maintain core product features' : 'meet the day-to-day demands of this role'}.`;
      reqExplanationCache.set(cacheKey, fallback);
      return fallback;
    }

    const prompt = `You are a career advisor. For the job role "${jobTitle}" at "${company}", explain in 1-2 concise sentences why this requirement matters: "${req}". Be specific. Under 30 words. No bullet points. Company context: ${(jobDescription || '').slice(0, 200)}`;
    ...
    const text = (choices && choices[0]?.message?.content?.trim()) || 'Key skill the employer is looking for in this role.';
...
// ── ATS Match Card — FIXED: passes correct description to backend ──────────────
// ── Drop-in replacement for ATSMatchCard in Applications.js ──────────────────
// Shows real score + matched keywords (green) + missing keywords (red)
...
function ATSMatchCard({ job, userId, dark }) {
  ...
  const handleCalculate = async () => {
    ...
    try {
      const jobDescription = (job.description || job.ai_summary || '').slice(0, 3000);
      if (!jobDescription.trim()) {
        setError('Job has no description. Cannot analyze.');
        setLoading(false);
        return;
      }
      ...
      setError(typeof d === 'string' ? d : 'Upload your resume in Settings → Document Vault first.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (scoreData) {
    ...
    const missing = scoreData.missing || [];
    const color = s >= 70 ? C.green : s >= 45 ? C.orange : C.red;
    const bgColor = s >= 70 ? C.greenL : s >= 45 ? C.orangeL : C.redL;
    const label = s >= 70 ? 'Strong Match' : s >= 45 ? 'Partial Match' : 'Low Match';

    return (
      <div style={{
        marginBottom: 24, borderRadius: 18, border: `1px solid ${color}30`,
        background: bgColor, overflow: 'hidden', animation: 'skelFadeIn 0.4s ease'
      }}>
        ...
        <div style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{
            ...
            <svg viewBox="0 0 36 36" style={{ position: 'absolute', inset: 0, transform: 'rotate(-90deg)' }}>
              <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                fill="none" stroke={`${color}30`} strokeWidth="3" />
              <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                fill="none" stroke={color} strokeWidth="3"
                strokeDasharray={`${s}, 100`}
                style={{ transition: 'stroke-dasharray 1s ease-out' }} />
            </svg>
            <span style={{ fontSize: '1rem', fontWeight: 700, color }}>{s}%</span>
          </div>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: '0.85rem', color: C.text, lineHeight: 1.5, margin: 0, fontWeight: 400 }}>
              {scoreData.feedback}
            </p>
          </div>
        </div>
        ...
        {matched.length > 0 && (
          <div style={{ padding: '0 20px 16px' }}>
            <div style={{
              fontSize: '0.6rem', fontWeight: 700, color: C.green,
              letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8
            }}>
              ✓ Matched ({matched.length})
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {matched.slice(0, 12).map((kw, i) => (
                <span key={i} style={{
                  padding: '3px 10px', borderRadius: 6,
                  background: 'rgba(16,185,129,0.12)', color: C.green,
                  fontSize: '0.72rem', fontWeight: 600, border: '1px solid rgba(16,185,129,0.2)'
                }}>
                  {kw}
                </span>
              ))}
        ...
        {missing.length > 0 && (
          <div style={{ padding: '0 20px 20px' }}>
            <div style={{
              fontSize: '0.6rem', fontWeight: 700, color: C.red,
              letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8
            }}>
              ✗ Missing ({missing.length})
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {missing.slice(0, 12).map((kw, i) => (
                <span key={i} style={{
                  padding: '3px 10px', borderRadius: 6,
                  background: 'rgba(239,68,68,0.08)', color: C.red,
                  fontSize: '0.72rem', fontWeight: 600, border: '1px solid rgba(239,68,68,0.2)'
                }}>
                  {kw}
                </span>
              ))}
        ...
        {/* Re-analyze */}
        <div style={{ padding: '0 20px 16px' }}>
          <button onClick={() => setScoreData(null)}
            style={{
              padding: '4px 14px', borderRadius: 8, background: 'transparent',
              border: `1px solid ${color}40`, color, fontSize: '0.72rem',
  ...
  return (
    <div style={{
      marginBottom: 24, padding: '16px', background: C.surfaceL,
      borderRadius: 16, border: `1px solid ${C.border}`,
      display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      flexWrap: 'wrap', gap: 12
    }}>
      <div>
        <div style={{ fontSize: '0.85rem', fontWeight: 600, color: C.text }}>ATS Resume Match</div>
        <div style={{ fontSize: '0.75rem', color: C.textM, marginTop: 2 }}>
          Real keyword analysis against this job description.
        </div>
      ...
      <div style={{ background: C.surface, borderRadius: 24, width: '100%', maxWidth: 680, maxHeight: '90vh', overflowY: 'auto', border: `1px solid ${C.border}`, boxShadow: dark ? '0 40px 100px rgba(0,0,0,0.6)' : '0 40px 100px rgba(0,0,0,0.12)', animation: 'modalIn 0.35s cubic-bezier(0.16, 1, 0.3, 1)' }}>
        ...
        <div style={{ padding: '20px 24px 28px' }}>
          ...
          {summary ? (
            ...
            // ADDED: Show raw description if no AI summary yet
            <div style={{ marginBottom: 22, padding: '18px', background: C.surfaceLow, borderRadius: 16, border: `1px solid ${C.border}` }}>
              <div style={{ fontSize: '0.58rem', fontWeight: 700, color: C.textL, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 10 }}>Job Description</div>
              <p style={{ fontSize: '0.875rem', color: C.text, lineHeight: 1.75, margin: 0, fontWeight: 300 }}>{job.description.slice(0, 500)}{job.description.length > 500 ? '...' : ''}</p>
            </div>
          ) : null}

          {/* Requirements — with AI tooltip */}
          {reqs.length > 0 && (
                ...
                <>
                  ...
                  <div className="jobs-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
                    {visibleJobs.map((job, idx) => {
                      ...
                      const ab = abbr(job.company || '');
                      const match = findMatch(job);
                      // FIXED: show ai_summary OR fallback to description snippet
                      const summary = cleanSummary(job.ai_summary) || (job.description ? job.description.slice(0, 120) + '...' : '');
                      return (
                        <div key={job.id}
