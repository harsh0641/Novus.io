    ...
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a senior career intelligence analyst. Be concise, specific, and direct. Never start with "Here is" or "This role" or "Summary". Jump straight to the insight. No bullet points.',
          },
          {
            role: 'user',
            content: `Write a 2-3 sentence intelligence brief for this specific job posting. Cover: what makes this role unique at ${job.company || 'this company'}, the ideal candidate profile, and one notable signal (culture, tech stack, compensation, or growth trajectory). Be concrete and job-specific — not generic.\n\nRole: ${job.title}\nCompany: ${job.company}\nLocation: ${job.location || 'N/A'}\nSalary: ${job.salary || 'Not disclosed'}\nPosting: ${desc.slice(0, 700)}`,
          },
        ],
      }),
    });
...
async function fetchHoverInsight(job) {
  const cacheKey = `hover__${job.company || ''}__${job.title || ''}`;
    ...
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Career intelligence agent. Be specific and actionable. No preamble. No bullet headers.',
          },
          {
            role: 'user',
            content: `For "${job.title}" at "${job.company}" — give 3 specific data points: (1) compensation market-rate assessment based on salary ${job.salary || 'undisclosed'}, (2) likely interview process/difficulty, (3) one concrete red or green flag from the job description. Keep each point to one sentence. Based on: ${desc.slice(0, 500)}`,
          },
        ],
      }),
    });
...
function JobModal({ job, onClose, onApply, isApplied, isSaving, dark, userId }) {
  ...
  // Fetch Groq intelligence whenever job changes
  useEffect(() => {
    if (!job) return;
    setAiIntel('');
    setDeepInsight('');
  ...
  // Hover handler: fetch deep insight on first hover
    ...
    >
      <div style={{ background: C.surface, borderRadius: 24, width: '100%', maxWidth: 680, maxHeight: '90vh', overflowY: 'auto', border: `1px solid ${C.border}`, boxShadow: dark ? '0 40px 100px rgba(0,0,0,0.6)' : '0 40px 100px rgba(0,0,0,0.12)', animation: 'modalIn 0.35s cubic-bezier(0.16, 1, 0.3, 1)' }}>
        ...
        <div style={{ padding: '20px 24px 28px' }}>
          ...
          {/* ── AI Intelligence Summary — Groq-powered, dynamic per job, hover for deep insights ── */}
          <div style={{ marginBottom: 22 }}>
            <div
              style={{
                padding: '18px',
                background: sumHovered
                  ? (dark ? 'rgba(59,130,246,0.07)' : '#f0f6ff')
                  : C.surfaceLow,
                borderRadius: 16,
                border: `1px solid ${sumHovered ? 'rgba(59,130,246,0.2)' : C.border}`,
            ...
            >
              ...
              {/* Main intelligence text — shows Groq result when ready, falls back to existing summary while loading */}
              ...
              {!sumHovered && !intelLoading && (
                <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 5, opacity: 0.4 }}>
...
export default function Applications() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: C.bg, transition: 'background 0.4s cubic-bezier(0.16,1,0.3,1)', fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif" }}>
      ...
      {/* Mobile sidebar overlay — FIX: display is now conditional, not hardcoded none */}
      {sidebarOpen && (
        <div
      ...
      {/* Sidebar — passes open/onClose so Sidebar component can handle mobile slide */}
