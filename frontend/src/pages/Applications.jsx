...
// Uses Claude API to generate context-aware requirement explanations per job
...
// ─────────────────────────────────────────────────────────────────────────────
// ADDED: Groq-powered per-job intelligence using existing REACT_APP_GROQ_API_KEY
// Two separate caches — one for main summary, one for hover deep insight
// ─────────────────────────────────────────────────────────────────────────────
const jobIntelligenceCache = new Map();
const hoverInsightCache = new Map();

async function fetchJobIntelligence(job) {
  const cacheKey = `groq_intel__${job.company || ''}__${job.title || ''}`;
  if (jobIntelligenceCache.has(cacheKey)) return jobIntelligenceCache.get(cacheKey);
  const desc = [job.description, job.ai_summary, job.requirements].filter(Boolean).join(' ');
  try {
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a senior career intelligence analyst. Be concise and specific to this exact job. Never start with "Here is" or "This role" or "Summary:". Jump straight to the insight. No bullet points.',
          },
          {
            role: 'user',
            content: `Write a 2-3 sentence intelligence brief for this specific job posting. Cover: what makes this role unique at ${job.company || 'this company'}, the ideal candidate profile, and one concrete signal (tech stack, growth stage, culture, or compensation trend). Be job-specific — never generic.\n\nRole: ${job.title}\nCompany: ${job.company}\nLocation: ${job.location || 'N/A'}\nSalary: ${job.salary || 'Not disclosed'}\nJob context: ${desc.slice(0, 700)}`,
          },
        ],
      }),
    });
    ...
    if (text) jobIntelligenceCache.set(cacheKey, text);
    return text;
  } catch {
    return '';
  }
...
async function fetchHoverInsight(job) {
  const cacheKey = `groq_hover__${job.company || ''}__${job.title || ''}`;
  if (hoverInsightCache.has(cacheKey)) return hoverInsightCache.get(cacheKey);
  const desc = [job.description, job.ai_summary, job.requirements].filter(Boolean).join(' ');
  try {
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Career intelligence agent. Specific and actionable. No preamble. No bullet headers. Write in plain sentences.',
          },
          {
            role: 'user',
            content: `For "${job.title}" at "${job.company}" — give exactly 3 data points in plain sentences: (1) compensation market-rate assessment for salary ${job.salary || 'undisclosed'}, (2) likely interview process and difficulty, (3) one concrete red or green flag from this specific job description. One sentence each. Based on: ${desc.slice(0, 500)}`,
          },
        ],
      }),
    });
    const data = await res.json();
    const text = (data.choices?.[0]?.message?.content || '').trim();
    if (text) hoverInsightCache.set(cacheKey, text);
    return text;
  } catch {
    return '';
  }
...
function ReqTimeline({ reqs, dark, job }) {
...
function JobModal({ job, onClose, onApply, isApplied, isSaving, dark, userId }) {
  ...
  // ADDED: Groq AI intelligence state — unique per job, resets on job change
  const [aiIntel, setAiIntel] = useState('');
  const [intelLoading, setIntelLoading] = useState(false);
  const [sumHovered, setSumHovered] = useState(false);
  ...
  // ADDED: fetch Groq intelligence on mount, keyed to this specific job
  useEffect(() => {
    if (!job) return;
    setAiIntel('');
    setDeepInsight('');
    ...
    fetchJobIntelligence(job).then(text => {
      setAiIntel(text);
      setIntelLoading(false);
    });
  }, [job?.id, job?.title, job?.company]); // eslint-disable-line

  // ADDED: on hover — fetch deep insight on first hover only, cached after
  const handleSumEnter = async () => {
    setSumHovered(true);
    if (deepInsight || deepLoading) return;
    setDeepLoading(true);
    const insight = await fetchHoverInsight(job);
    ...
    >
      <div style={{ background: C.surface, borderRadius: 24, width: '100%', maxWidth: 680, maxHeight: '90vh', overflowY: 'auto', border: `1px solid ${C.border}`, boxShadow: dark ? '0 40px 100px rgba(0,0,0,0.6)' : '0 40px 100px rgba(0,0,0,0.12)', animation: 'modalIn 0.35s cubic-bezier(0.16, 1, 0.3, 1)' }}>
        ...
        <div style={{ padding: '20px 24px 28px' }}>
          ...
          {/* ADDED: AI Intelligence Summary — Groq-powered, per-job dynamic, hover reveals deep insights */}
          {/* Existing static summary shown as fallback while Groq loads. No original code removed. */}
          <div style={{ marginBottom: 22 }}>
            <div
              style={{
                padding: '18px',
                background: sumHovered ? (dark ? 'rgba(59,130,246,0.06)' : '#f0f6ff') : C.surfaceLow,
                borderRadius: 16,
                border: `1px solid ${sumHovered ? 'rgba(59,130,246,0.18)' : C.border}`,
                transition: 'all 0.3s cubic-bezier(0.16,1,0.3,1)',
                cursor: 'default',
                position: 'relative',
              }}
              onMouseEnter={handleSumEnter}
              onMouseLeave={() => setSumHovered(false)}
            >
              {/* Label row + live status */}
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ fontSize: '0.58rem', fontWeight: 700, color: C.textL, letterSpacing: '0.12em', textTransform: 'uppercase' }}>Intelligence Summary</div>
                {intelLoading && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#3b82f6', animation: 'reqPulse 1s ease-in-out infinite' }} />
                    <span style={{ fontSize: '0.6rem', color: '#3b82f6', fontWeight: 600, letterSpacing: '0.08em' }}>AI analyzing…</span>
                  </div>
                )}
                {!intelLoading && aiIntel && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#10b981' }} />
                    <span style={{ fontSize: '0.6rem', color: '#10b981', fontWeight: 600, letterSpacing: '0.08em' }}>Groq · llama-3.1</span>
                  </div>
                )}
              </div>

              {/* Dynamic per-job text — Groq result replaces static summary once ready */}
              <p style={{ fontSize: '0.875rem', color: C.text, lineHeight: 1.75, margin: 0, fontWeight: 300, transition: 'opacity 0.4s ease', opacity: intelLoading ? 0.5 : 1 }}>
                {intelLoading
                  ? (summary || 'Generating intelligence analysis for this role…')
                  : (aiIntel || summary || 'No summary available for this posting.')}
              </p>

              {/* Hover: deep insight panel revealed on hover */}
              {sumHovered && (
                <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${C.border}`, animation: 'tooltipFade 0.25s cubic-bezier(0.16,1,0.3,1)' }}>
                  <div style={{ fontSize: '0.58rem', fontWeight: 700, color: '#3b82f6', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 8 }}>Deep Intelligence</div>
                  {deepLoading ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#3b82f6', animation: 'reqPulse 1s ease-in-out infinite' }} />
                      <span style={{ fontSize: '0.78rem', color: C.textM, fontStyle: 'italic' }}>AI fetching deep insights for this role…</span>
                    </div>
                  ) : deepInsight ? (
                    <p style={{ fontSize: '0.8rem', color: C.textM, lineHeight: 1.65, margin: 0, fontWeight: 300 }}>{deepInsight}</p>
                  ) : null}
                </div>
              )}

              {/* Subtle hint when idle */}
              {!sumHovered && !intelLoading && (
                <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 5, opacity: 0.35 }}>
                  <svg width="10" height="10" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4M12 8h.01"/></svg>
                  <span style={{ fontSize: '0.6rem', color: C.textL, letterSpacing: '0.06em' }}>Hover for deep insights</span>
                </div>
              )}
            </div>
          </div>
...
export default function Applications() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: C.bg, transition: 'background 0.4s cubic-bezier(0.16,1,0.3,1)', fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif" }}>
      ...
      {/* Mobile sidebar overlay — FIXED: removed hardcoded display:'none' so CSS media query works */}
      {sidebarOpen && (
        <div
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, position: 'relative', zIndex: 10, marginLeft: 'var(--sidebar-width, 280px)', transition: 'margin-left 0.5s cubic-bezier(0.16, 1, 0.3, 1)' }} className="main-content">
        ...
        <header style={{ position: 'sticky', top: 0, zIndex: 40, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 24px', background: dark ? 'rgba(3,3,3,0.75)' : 'rgba(244,244,245,0.85)', backdropFilter: 'blur(24px) saturate(180%)', borderBottom: `1px solid ${C.border}`, WebkitBackdropFilter: 'blur(24px) saturate(180%)', gap: 16 }}>
          ...
          <button
            ...
            style={{ display: 'none', width: 36, height: 36, borderRadius: 9, border: `1px solid ${C.border}`, background: 'transparent', cursor: 'pointer', color: C.text, alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
          >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
            <div style={{ textAlign: 'right' }} className="header-user-info">
              ...
              <div style={{ fontSize: '0.65rem', color: C.textL, letterSpacing: '0.05em', marginTop: 1 }}>
                {/* FIX: "AI Comms Active" label as seen in screenshot */}
                AI Comms Active
              </div>
            </div>
